module ticketchain::admin_registry {
    ...
    const E_ALREADY_SUPERADMIN: u64 = 6;
    const E_CANNOT_REMOVE_LAST_SUPERADMIN: u64 = 7;
    const E_ADMIN_MANAGEMENT_DISABLED: u64 = 8;
    ...
    public entry fun initialize(superadmin: &signer) {
        ...
        let mut superadmins = vector::empty<address>();
    ...
    public entry fun add_superadmin(
        ...
        assert!(is_in_vector(&registry.superadmins, caller_addr), E_NOT_SUPERADMIN);
        assert!(!is_in_vector(&registry.superadmins, new_superadmin), E_ALREADY_SUPERADMIN);
        
        vector::push_back(&mut registry.superadmins, new_superadmin);
        
        if (!is_in_vector(&registry.admins, new_superadmin)) {
    ...
    public entry fun remove_superadmin(
        ...
        assert!(is_in_vector(&registry.superadmins, superadmin_to_remove), E_NOT_SUPERADMIN);
    ...
    public entry fun transfer_superadmin(
        ...
        if (!is_in_vector(&registry.superadmins, new_superadmin)) {
            vector::push_back(&mut registry.superadmins, new_superadmin);
        };
    ...
    public entry fun add_admin(
        ...
        let is_superadmin = is_in_vector(&registry.superadmins, caller_addr);
        let is_admin = is_in_vector(&registry.admins, caller_addr);
        
        if (!is_superadmin) {
            assert!(registry.allow_admin_self_service && is_admin, E_ADMIN_MANAGEMENT_DISABLED);
        };
        
        assert!(!is_in_vector(&registry.admins, new_admin), E_ALREADY_ADMIN);
    ...
    public entry fun remove_admin(
        ...
        assert!(is_in_vector(&registry.superadmins, caller_addr), E_NOT_SUPERADMIN);
        assert!(!is_in_vector(&registry.superadmins, admin_to_remove), E_CANNOT_REMOVE_SUPERADMIN);
        assert!(is_in_vector(&registry.admins, admin_to_remove), E_NOT_ADMIN);
    ...
    public fun is_admin(registry_address: address, addr: address): bool acquires AdminRegistry {
        ...
        is_in_vector(&registry.admins, addr)
    }
    ...
    fun is_in_vector(vec: &vector<address>, addr: address): bool {
    ...
    fun copy_vector(vec: &vector<address>): vector<address> {
